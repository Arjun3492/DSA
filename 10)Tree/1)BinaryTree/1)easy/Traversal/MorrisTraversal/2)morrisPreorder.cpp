...
//  Problem Statement
//  Given the root of a binary tree, return the preorder traversal of its nodes' values using the Morris Traversal technique.

// Link : https://www.naukri.com/code360/problems/preorder-binary-tree_5948

// Approach :
// The idea of Morris Traversal is based on Threaded Binary Tree. In this traversal, we first create links to Inorder successor and print the data using these links, and finally revert the changes to restore the original tree.
...
// 2.1 We will find the rightmost node of the left subtree and point its right pointer to the current node ,print the current node and  move to the left child .
// 2.2 If the right pointer of the rightmost node already points to the current node, we will revert the changes and move to the right child.
    ...
    while (curr != nullptr)
    {
        ...
        else
        {
            ...
            if (prev->right == nullptr)
            {
                ...
                prev->right = curr;
                // Print the current node and move to the left child
                ans.push_back(curr->data);
                curr = curr->left;
            }
