...
vector<int> alternateNumbers(vector<int> &a)
{
    ...
    // storing the positive and negative elements in separate arrays
    for (int i : a)
    {
        if (i < 0)
            neg.push_back(i);
    ...
    // placing the elements alternatively in the ans array
    for (int i = 0; i < n / 2; i++)
    {
        ans.push_back(pos[i]);
        ans.push_back(neg[i]);
...
// tc:O(n)
// sc:O(1) as no extra space is used, but we are using array arr of size n to store the answer then sc:O(n)
vector<int> alternateNumbers(vector<int> &a)
{
    int n = a.size();
    vector<int> arr(n, 0);
